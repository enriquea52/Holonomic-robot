...
def optimisar(trayectoria,obstaculo):
    costos_euler=[0]
    for i in range(len(trayectoria)-1):
        nodo_a=trayectoria[i]
        nodo_b=trayectoria[i+1]
        costo_eu=math.sqrt(math.pow(nodo_b[0]-nodo_a[0],2)+math.pow(nodo_b[1]-nodo_a[1],2))+costos_euler[len(costos_euler)-1]
        costos_euler.append(costo_eu)
    nueva_ruta=[trayectoria[0]]
    Posición=0
    final=trayectoria[len(trayectoria)-1]
    for i in range(len(trayectoria)-1):
        a=0
        b=0
        if trayectoria[Posición]==final:
            return nueva_ruta
        for j in range(len(trayectoria)):
            if j==Posición+1:
                nodo_A=trayectoria[Posición]
                nodo_B=trayectoria[j]
                prueba=secciones(nodo_A,nodo_B,obstaculo)
                if prueba==1:
                    a=j
                if a!=0:
                    nueva_ruta.append(trayectoria[a])
                    Posición=a
